...
//       
//       Creation Date:  Wed Nov 23 16:14:57 2022
//       Version: 	r5p0
//
...
`ifdef POWER_PINS
module sram_keybuf1 (VDDCE, VDDPE, VSSE, CENYA, WENYA, AYA, DYA, CENYB, WENYB, AYB,
...
`else
module sram_keybuf1 (CENYA, WENYA, AYA, DYA, CENYB, WENYB, AYB, DYB, QA, QB, CLKA,
...
`endif
  ...
  reg LAST_CLKB;
  reg [51:0] QA_int;
  reg [51:0] QA_int_delayed;
  reg [51:0] QB_int;
  reg [51:0] QB_int_delayed;
  reg [51:0] writeEnable;
  real previous_CLKA;
  real previous_CLKB;
  ...
  reg NOT_DA2, NOT_DA1, NOT_DA0, NOT_CENB, NOT_WENB, NOT_AB6, NOT_AB5, NOT_AB4, NOT_AB3;
  reg NOT_AB2, NOT_AB1, NOT_AB0, NOT_DB51, NOT_DB50, NOT_DB49, NOT_DB48, NOT_DB47;
  ...
  reg NOT_EMAWB1, NOT_EMAWB0, NOT_EMASB, NOT_TENA, NOT_TCENA, NOT_TWENA, NOT_TAA6;
  reg NOT_TAA5, NOT_TAA4, NOT_TAA3, NOT_TAA2, NOT_TAA1, NOT_TAA0, NOT_TDA51, NOT_TDA50;
  ...
  reg NOT_TCENB, NOT_TWENB, NOT_TAB6, NOT_TAB5, NOT_TAB4, NOT_TAB3, NOT_TAB2, NOT_TAB1;
  reg NOT_TAB0, NOT_TDB51, NOT_TDB50, NOT_TDB49, NOT_TDB48, NOT_TDB47, NOT_TDB46, NOT_TDB45;
 ...
 wire  CLKB_;
  ...
  assign `ARM_UD_DP DYA_ = RET1N_ ? (TENA_ ? DA_ : TDA_) : {52{1'bx}};
  ...
  assign `ARM_UD_DP DYB_ = RET1N_ ? (TENB_ ? DB_ : TDB_) : {52{1'bx}};
  assign `ARM_UD_SEQ QA_ = RET1N_ ? (BENA_ ? ((STOVA_ ? (QA_int_delayed) : (QA_int))) : TQA_) : {52{1'bx}};
  assign `ARM_UD_SEQ QB_ = RET1N_ ? (BENB_ ? ((STOVB_ ? (QB_int_delayed) : (QB_int))) : TQB_) : {52{1'bx}};
  ...
  begin
	$readmemb(filename, memld);
     if (CENA_ === 1'b1 && CENB_ === 1'b1) begin
      ...
      row = mem[row_address];
        writeEnable = {52{1'b1}};
        row_mask =  ( {7'b0000000, writeEnable[51], 7'b0000000, writeEnable[50], 7'b0000000, writeEnable[49],
        ...
        new_data =  ( {7'b0000000, wordtemp[51], 7'b0000000, wordtemp[50], 7'b0000000, wordtemp[49],
  ...
  begin
	dump_file_desc = $fopen(filename_dump);
     if (CENA_ === 1'b1 && CENB_ === 1'b1) begin
      ...
      QA_int = {shifted_readLatch0[102], shifted_readLatch0[100], shifted_readLatch0[98],
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
     ...
     && !CENA_int)} === 1'bx) begin
      ...
      QA_int = WENA_int !== 1'b1 ? QA_int : {52{1'bx}};
      QA_int_delayed = WENA_int !== 1'b1 ? QA_int_delayed : {52{1'bx}};
      ...
      if (WENA_int !== 1'b1) begin
        row_mask =  ( {7'b0000000, writeEnable[51], 7'b0000000, writeEnable[50], 7'b0000000, writeEnable[49],
          ...
          7'b0000000, writeEnable[3], 7'b0000000, writeEnable[2], 7'b0000000, writeEnable[1],
          7'b0000000, writeEnable[0]} << mux_address);
        new_data =  ( {7'b0000000, DA_int[51], 7'b0000000, DA_int[50], 7'b0000000, DA_int[49],
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
     ...
     && !CENB_int)} === 1'bx) begin
      ...
      QB_int = WENB_int !== 1'b1 ? QB_int : {52{1'bx}};
      QB_int_delayed = WENB_int !== 1'b1 ? QB_int_delayed : {52{1'bx}};
      ...
      if (WENB_int !== 1'b1) begin
        row_mask =  ( {7'b0000000, writeEnable[51], 7'b0000000, writeEnable[50], 7'b0000000, writeEnable[49],
          ...
          7'b0000000, writeEnable[3], 7'b0000000, writeEnable[2], 7'b0000000, writeEnable[1],
          7'b0000000, writeEnable[0]} << mux_address);
        new_data =  ( {7'b0000000, DB_int[51], 7'b0000000, DB_int[50], 7'b0000000, DB_int[49],
      ...
      QB_int = {shifted_readLatch1[102], shifted_readLatch1[100], shifted_readLatch1[98],
        shifted_readLatch1[96], shifted_readLatch1[94], shifted_readLatch1[92], shifted_readLatch1[90],
        shifted_readLatch1[88], shifted_readLatch1[86], shifted_readLatch1[84], shifted_readLatch1[82],
        shifted_readLatch1[80], shifted_readLatch1[78], shifted_readLatch1[76], shifted_readLatch1[74],
        shifted_readLatch1[72], shifted_readLatch1[70], shifted_readLatch1[68], shifted_readLatch1[66],
        shifted_readLatch1[64], shifted_readLatch1[62], shifted_readLatch1[60], shifted_readLatch1[58],
        shifted_readLatch1[56], shifted_readLatch1[54], shifted_readLatch1[52], shifted_readLatch1[50],
        shifted_readLatch1[48], shifted_readLatch1[46], shifted_readLatch1[44], shifted_readLatch1[42],
        shifted_readLatch1[40], shifted_readLatch1[38], shifted_readLatch1[36], shifted_readLatch1[34],
        shifted_readLatch1[32], shifted_readLatch1[30], shifted_readLatch1[28], shifted_readLatch1[26],
...
`endif
 ...
 wire  CLKB_;
  ...
  buf B61(CENYB, CENYB_);
  buf B62(WENYB, WENYB_);
  buf B63(AYB[0], AYB_[0]);
  buf B64(AYB[1], AYB_[1]);
  buf B65(AYB[2], AYB_[2]);
  buf B66(AYB[3], AYB_[3]);
  buf B67(AYB[4], AYB_[4]);
  buf B68(AYB[5], AYB_[5]);
  buf B69(AYB[6], AYB_[6]);
  buf B70(DYB[0], DYB_[0]);
  buf B71(DYB[1], DYB_[1]);
  buf B72(DYB[2], DYB_[2]);
  buf B73(DYB[3], DYB_[3]);
  buf B74(DYB[4], DYB_[4]);
  buf B75(DYB[5], DYB_[5]);
  buf B76(DYB[6], DYB_[6]);
  buf B77(DYB[7], DYB_[7]);
  buf B78(DYB[8], DYB_[8]);
  buf B79(DYB[9], DYB_[9]);
  buf B80(DYB[10], DYB_[10]);
  buf B81(DYB[11], DYB_[11]);
  buf B82(DYB[12], DYB_[12]);
  buf B83(DYB[13], DYB_[13]);
  buf B84(DYB[14], DYB_[14]);
  buf B85(DYB[15], DYB_[15]);
  buf B86(DYB[16], DYB_[16]);
  buf B87(DYB[17], DYB_[17]);
  buf B88(DYB[18], DYB_[18]);
  buf B89(DYB[19], DYB_[19]);
  buf B90(DYB[20], DYB_[20]);
  buf B91(DYB[21], DYB_[21]);
  buf B92(DYB[22], DYB_[22]);
  buf B93(DYB[23], DYB_[23]);
  buf B94(DYB[24], DYB_[24]);
  buf B95(DYB[25], DYB_[25]);
  buf B96(DYB[26], DYB_[26]);
  buf B97(DYB[27], DYB_[27]);
  buf B98(DYB[28], DYB_[28]);
  buf B99(DYB[29], DYB_[29]);
  buf B100(DYB[30], DYB_[30]);
  buf B101(DYB[31], DYB_[31]);
  buf B102(DYB[32], DYB_[32]);
  buf B103(DYB[33], DYB_[33]);
  buf B104(DYB[34], DYB_[34]);
  buf B105(DYB[35], DYB_[35]);
  buf B106(DYB[36], DYB_[36]);
  buf B107(DYB[37], DYB_[37]);
  buf B108(DYB[38], DYB_[38]);
  buf B109(DYB[39], DYB_[39]);
  buf B110(DYB[40], DYB_[40]);
  buf B111(DYB[41], DYB_[41]);
  buf B112(DYB[42], DYB_[42]);
  buf B113(DYB[43], DYB_[43]);
  buf B114(DYB[44], DYB_[44]);
  buf B115(DYB[45], DYB_[45]);
  buf B116(DYB[46], DYB_[46]);
  buf B117(DYB[47], DYB_[47]);
  buf B118(DYB[48], DYB_[48]);
  buf B119(DYB[49], DYB_[49]);
  buf B120(DYB[50], DYB_[50]);
  buf B121(DYB[51], DYB_[51]);
  buf B122(QA[0], QA_[0]);
  buf B123(QA[1], QA_[1]);
  buf B124(QA[2], QA_[2]);
  buf B125(QA[3], QA_[3]);
  ...
  buf B130(QA[8], QA_[8]);
  buf B131(QA[9], QA_[9]);
  buf B132(QA[10], QA_[10]);
  buf B133(QA[11], QA_[11]);
  buf B134(QA[12], QA_[12]);
  buf B135(QA[13], QA_[13]);
  buf B136(QA[14], QA_[14]);
  buf B137(QA[15], QA_[15]);
  buf B138(QA[16], QA_[16]);
  buf B139(QA[17], QA_[17]);
  buf B140(QA[18], QA_[18]);
  buf B141(QA[19], QA_[19]);
  buf B142(QA[20], QA_[20]);
  buf B143(QA[21], QA_[21]);
  buf B144(QA[22], QA_[22]);
  buf B145(QA[23], QA_[23]);
  buf B146(QA[24], QA_[24]);
  buf B147(QA[25], QA_[25]);
  buf B148(QA[26], QA_[26]);
  buf B149(QA[27], QA_[27]);
  buf B150(QA[28], QA_[28]);
  buf B151(QA[29], QA_[29]);
  buf B152(QA[30], QA_[30]);
  buf B153(QA[31], QA_[31]);
  buf B154(QA[32], QA_[32]);
  buf B155(QA[33], QA_[33]);
  buf B156(QA[34], QA_[34]);
  buf B157(QA[35], QA_[35]);
  buf B158(QA[36], QA_[36]);
  buf B159(QA[37], QA_[37]);
  buf B160(QA[38], QA_[38]);
  buf B161(QA[39], QA_[39]);
  buf B162(QA[40], QA_[40]);
  buf B163(QA[41], QA_[41]);
  buf B164(QA[42], QA_[42]);
  buf B165(QA[43], QA_[43]);
  buf B166(QA[44], QA_[44]);
  buf B167(QA[45], QA_[45]);
  buf B168(QA[46], QA_[46]);
  buf B169(QA[47], QA_[47]);
  buf B170(QA[48], QA_[48]);
  buf B171(QA[49], QA_[49]);
  buf B172(QA[50], QA_[50]);
  buf B173(QA[51], QA_[51]);
  buf B174(QB[0], QB_[0]);
  buf B175(QB[1], QB_[1]);
  buf B176(QB[2], QB_[2]);
  buf B177(QB[3], QB_[3]);
  ...
  buf B182(QB[8], QB_[8]);
  buf B183(QB[9], QB_[9]);
  buf B184(QB[10], QB_[10]);
  buf B185(QB[11], QB_[11]);
  buf B186(QB[12], QB_[12]);
  buf B187(QB[13], QB_[13]);
  buf B188(QB[14], QB_[14]);
  buf B189(QB[15], QB_[15]);
  buf B190(QB[16], QB_[16]);
  buf B191(QB[17], QB_[17]);
  buf B192(QB[18], QB_[18]);
  buf B193(QB[19], QB_[19]);
  buf B194(QB[20], QB_[20]);
  buf B195(QB[21], QB_[21]);
  buf B196(QB[22], QB_[22]);
  buf B197(QB[23], QB_[23]);
  buf B198(QB[24], QB_[24]);
  buf B199(QB[25], QB_[25]);
  buf B200(QB[26], QB_[26]);
  buf B201(QB[27], QB_[27]);
  buf B202(QB[28], QB_[28]);
  buf B203(QB[29], QB_[29]);
  buf B204(QB[30], QB_[30]);
  buf B205(QB[31], QB_[31]);
  buf B206(QB[32], QB_[32]);
  buf B207(QB[33], QB_[33]);
  buf B208(QB[34], QB_[34]);
  buf B209(QB[35], QB_[35]);
  buf B210(QB[36], QB_[36]);
  buf B211(QB[37], QB_[37]);
  buf B212(QB[38], QB_[38]);
  buf B213(QB[39], QB_[39]);
  buf B214(QB[40], QB_[40]);
  buf B215(QB[41], QB_[41]);
  buf B216(QB[42], QB_[42]);
  buf B217(QB[43], QB_[43]);
  buf B218(QB[44], QB_[44]);
  buf B219(QB[45], QB_[45]);
  buf B220(QB[46], QB_[46]);
  buf B221(QB[47], QB_[47]);
  buf B222(QB[48], QB_[48]);
  buf B223(QB[49], QB_[49]);
  buf B224(QB[50], QB_[50]);
  buf B225(QB[51], QB_[51]);
  buf B226(CLKA_, CLKA);
  buf B227(CENA_, CENA);
  buf B228(WENA_, WENA);
  buf B229(AA_[0], AA[0]);
  ...
  buf B244(DA_[8], DA[8]);
  buf B245(DA_[9], DA[9]);
  buf B246(DA_[10], DA[10]);
  buf B247(DA_[11], DA[11]);
  buf B248(DA_[12], DA[12]);
  buf B249(DA_[13], DA[13]);
  buf B250(DA_[14], DA[14]);
  buf B251(DA_[15], DA[15]);
  buf B252(DA_[16], DA[16]);
  buf B253(DA_[17], DA[17]);
  buf B254(DA_[18], DA[18]);
  buf B255(DA_[19], DA[19]);
  buf B256(DA_[20], DA[20]);
  buf B257(DA_[21], DA[21]);
  buf B258(DA_[22], DA[22]);
  buf B259(DA_[23], DA[23]);
  buf B260(DA_[24], DA[24]);
  buf B261(DA_[25], DA[25]);
  buf B262(DA_[26], DA[26]);
  buf B263(DA_[27], DA[27]);
  buf B264(DA_[28], DA[28]);
  buf B265(DA_[29], DA[29]);
  buf B266(DA_[30], DA[30]);
  buf B267(DA_[31], DA[31]);
  buf B268(DA_[32], DA[32]);
  buf B269(DA_[33], DA[33]);
  buf B270(DA_[34], DA[34]);
  buf B271(DA_[35], DA[35]);
  buf B272(DA_[36], DA[36]);
  buf B273(DA_[37], DA[37]);
  buf B274(DA_[38], DA[38]);
  buf B275(DA_[39], DA[39]);
  buf B276(DA_[40], DA[40]);
  buf B277(DA_[41], DA[41]);
  buf B278(DA_[42], DA[42]);
  buf B279(DA_[43], DA[43]);
  buf B280(DA_[44], DA[44]);
  buf B281(DA_[45], DA[45]);
  buf B282(DA_[46], DA[46]);
  buf B283(DA_[47], DA[47]);
  buf B284(DA_[48], DA[48]);
  buf B285(DA_[49], DA[49]);
  buf B286(DA_[50], DA[50]);
  buf B287(DA_[51], DA[51]);
  buf B288(CLKB_, CLKB);
  buf B289(CENB_, CENB);
  buf B290(WENB_, WENB);
  buf B291(AB_[0], AB[0]);
  ...
  buf B350(EMAA_[0], EMAA[0]);
  buf B351(EMAA_[1], EMAA[1]);
  buf B352(EMAA_[2], EMAA[2]);
  buf B353(EMAWA_[0], EMAWA[0]);
  buf B354(EMAWA_[1], EMAWA[1]);
  buf B355(EMASA_, EMASA);
  buf B356(EMAB_[0], EMAB[0]);
  buf B357(EMAB_[1], EMAB[1]);
  buf B358(EMAB_[2], EMAB[2]);
  buf B359(EMAWB_[0], EMAWB[0]);
  buf B360(EMAWB_[1], EMAWB[1]);
  buf B361(EMASB_, EMASB);
  buf B362(TENA_, TENA);
  buf B363(BENA_, BENA);
  buf B364(TCENA_, TCENA);
  buf B365(TWENA_, TWENA);
  buf B366(TAA_[0], TAA[0]);
  buf B367(TAA_[1], TAA[1]);
  buf B368(TAA_[2], TAA[2]);
  buf B369(TAA_[3], TAA[3]);
  buf B370(TAA_[4], TAA[4]);
  buf B371(TAA_[5], TAA[5]);
  buf B372(TAA_[6], TAA[6]);
  buf B373(TDA_[0], TDA[0]);
  buf B374(TDA_[1], TDA[1]);
  buf B375(TDA_[2], TDA[2]);
  buf B376(TDA_[3], TDA[3]);
  buf B377(TDA_[4], TDA[4]);
  buf B378(TDA_[5], TDA[5]);
  buf B379(TDA_[6], TDA[6]);
  buf B380(TDA_[7], TDA[7]);
  buf B381(TDA_[8], TDA[8]);
  buf B382(TDA_[9], TDA[9]);
  buf B383(TDA_[10], TDA[10]);
  buf B384(TDA_[11], TDA[11]);
  buf B385(TDA_[12], TDA[12]);
  buf B386(TDA_[13], TDA[13]);
  buf B387(TDA_[14], TDA[14]);
  buf B388(TDA_[15], TDA[15]);
  buf B389(TDA_[16], TDA[16]);
  buf B390(TDA_[17], TDA[17]);
  buf B391(TDA_[18], TDA[18]);
  buf B392(TDA_[19], TDA[19]);
  buf B393(TDA_[20], TDA[20]);
  buf B394(TDA_[21], TDA[21]);
  buf B395(TDA_[22], TDA[22]);
  buf B396(TDA_[23], TDA[23]);
  buf B397(TDA_[24], TDA[24]);
  buf B398(TDA_[25], TDA[25]);
  buf B399(TDA_[26], TDA[26]);
  buf B400(TDA_[27], TDA[27]);
  buf B401(TDA_[28], TDA[28]);
  buf B402(TDA_[29], TDA[29]);
  buf B403(TDA_[30], TDA[30]);
  buf B404(TDA_[31], TDA[31]);
  buf B405(TDA_[32], TDA[32]);
  buf B406(TDA_[33], TDA[33]);
  buf B407(TDA_[34], TDA[34]);
  buf B408(TDA_[35], TDA[35]);
  buf B409(TDA_[36], TDA[36]);
  buf B410(TDA_[37], TDA[37]);
  buf B411(TDA_[38], TDA[38]);
  buf B412(TDA_[39], TDA[39]);
  buf B413(TDA_[40], TDA[40]);
  buf B414(TDA_[41], TDA[41]);
  buf B415(TDA_[42], TDA[42]);
  buf B416(TDA_[43], TDA[43]);
  buf B417(TDA_[44], TDA[44]);
  buf B418(TDA_[45], TDA[45]);
  buf B419(TDA_[46], TDA[46]);
  buf B420(TDA_[47], TDA[47]);
  buf B421(TDA_[48], TDA[48]);
  buf B422(TDA_[49], TDA[49]);
  buf B423(TDA_[50], TDA[50]);
  buf B424(TDA_[51], TDA[51]);
  buf B425(TQA_[0], TQA[0]);
  buf B426(TQA_[1], TQA[1]);
  buf B427(TQA_[2], TQA[2]);
  buf B428(TQA_[3], TQA[3]);
  buf B429(TQA_[4], TQA[4]);
  buf B430(TQA_[5], TQA[5]);
  buf B431(TQA_[6], TQA[6]);
  buf B432(TQA_[7], TQA[7]);
  buf B433(TQA_[8], TQA[8]);
  buf B434(TQA_[9], TQA[9]);
  buf B435(TQA_[10], TQA[10]);
  buf B436(TQA_[11], TQA[11]);
  buf B437(TQA_[12], TQA[12]);
  buf B438(TQA_[13], TQA[13]);
  buf B439(TQA_[14], TQA[14]);
  buf B440(TQA_[15], TQA[15]);
  buf B441(TQA_[16], TQA[16]);
  buf B442(TQA_[17], TQA[17]);
  buf B443(TQA_[18], TQA[18]);
  buf B444(TQA_[19], TQA[19]);
  buf B445(TQA_[20], TQA[20]);
  buf B446(TQA_[21], TQA[21]);
  buf B447(TQA_[22], TQA[22]);
  buf B448(TQA_[23], TQA[23]);
  buf B449(TQA_[24], TQA[24]);
  buf B450(TQA_[25], TQA[25]);
  buf B451(TQA_[26], TQA[26]);
  buf B452(TQA_[27], TQA[27]);
  buf B453(TQA_[28], TQA[28]);
  buf B454(TQA_[29], TQA[29]);
  buf B455(TQA_[30], TQA[30]);
  buf B456(TQA_[31], TQA[31]);
  buf B457(TQA_[32], TQA[32]);
  buf B458(TQA_[33], TQA[33]);
  buf B459(TQA_[34], TQA[34]);
  buf B460(TQA_[35], TQA[35]);
  buf B461(TQA_[36], TQA[36]);
  buf B462(TQA_[37], TQA[37]);
  buf B463(TQA_[38], TQA[38]);
  buf B464(TQA_[39], TQA[39]);
  buf B465(TQA_[40], TQA[40]);
  buf B466(TQA_[41], TQA[41]);
  buf B467(TQA_[42], TQA[42]);
  buf B468(TQA_[43], TQA[43]);
  buf B469(TQA_[44], TQA[44]);
  buf B470(TQA_[45], TQA[45]);
  buf B471(TQA_[46], TQA[46]);
  buf B472(TQA_[47], TQA[47]);
  buf B473(TQA_[48], TQA[48]);
  buf B474(TQA_[49], TQA[49]);
  buf B475(TQA_[50], TQA[50]);
  buf B476(TQA_[51], TQA[51]);
  buf B477(TENB_, TENB);
  buf B478(BENB_, BENB);
  buf B479(TCENB_, TCENB);
  buf B480(TWENB_, TWENB);
  buf B481(TAB_[0], TAB[0]);
  buf B482(TAB_[1], TAB[1]);
  buf B483(TAB_[2], TAB[2]);
  buf B484(TAB_[3], TAB[3]);
  buf B485(TAB_[4], TAB[4]);
  buf B486(TAB_[5], TAB[5]);
  buf B487(TAB_[6], TAB[6]);
  buf B488(TDB_[0], TDB[0]);
  buf B489(TDB_[1], TDB[1]);
  buf B490(TDB_[2], TDB[2]);
  buf B491(TDB_[3], TDB[3]);
  buf B492(TDB_[4], TDB[4]);
  buf B493(TDB_[5], TDB[5]);
  buf B494(TDB_[6], TDB[6]);
  buf B495(TDB_[7], TDB[7]);
  buf B496(TDB_[8], TDB[8]);
  buf B497(TDB_[9], TDB[9]);
  buf B498(TDB_[10], TDB[10]);
  buf B499(TDB_[11], TDB[11]);
  buf B500(TDB_[12], TDB[12]);
  buf B501(TDB_[13], TDB[13]);
  buf B502(TDB_[14], TDB[14]);
  buf B503(TDB_[15], TDB[15]);
  buf B504(TDB_[16], TDB[16]);
  buf B505(TDB_[17], TDB[17]);
  buf B506(TDB_[18], TDB[18]);
  buf B507(TDB_[19], TDB[19]);
  buf B508(TDB_[20], TDB[20]);
  buf B509(TDB_[21], TDB[21]);
  buf B510(TDB_[22], TDB[22]);
  buf B511(TDB_[23], TDB[23]);
  buf B512(TDB_[24], TDB[24]);
  buf B513(TDB_[25], TDB[25]);
  buf B514(TDB_[26], TDB[26]);
  buf B515(TDB_[27], TDB[27]);
  buf B516(TDB_[28], TDB[28]);
  buf B517(TDB_[29], TDB[29]);
  buf B518(TDB_[30], TDB[30]);
  buf B519(TDB_[31], TDB[31]);
  buf B520(TDB_[32], TDB[32]);
  buf B521(TDB_[33], TDB[33]);
  buf B522(TDB_[34], TDB[34]);
  buf B523(TDB_[35], TDB[35]);
  buf B524(TDB_[36], TDB[36]);
  buf B525(TDB_[37], TDB[37]);
  buf B526(TDB_[38], TDB[38]);
  buf B527(TDB_[39], TDB[39]);
  buf B528(TDB_[40], TDB[40]);
  buf B529(TDB_[41], TDB[41]);
  buf B530(TDB_[42], TDB[42]);
  buf B531(TDB_[43], TDB[43]);
  buf B532(TDB_[44], TDB[44]);
  buf B533(TDB_[45], TDB[45]);
  buf B534(TDB_[46], TDB[46]);
  buf B535(TDB_[47], TDB[47]);
  buf B536(TDB_[48], TDB[48]);
  buf B537(TDB_[49], TDB[49]);
  buf B538(TDB_[50], TDB[50]);
  buf B539(TDB_[51], TDB[51]);
  buf B540(TQB_[0], TQB[0]);
  buf B541(TQB_[1], TQB[1]);
  buf B542(TQB_[2], TQB[2]);
  buf B543(TQB_[3], TQB[3]);
  buf B544(TQB_[4], TQB[4]);
  buf B545(TQB_[5], TQB[5]);
  buf B546(TQB_[6], TQB[6]);
  buf B547(TQB_[7], TQB[7]);
  buf B548(TQB_[8], TQB[8]);
  buf B549(TQB_[9], TQB[9]);
  buf B550(TQB_[10], TQB[10]);
  buf B551(TQB_[11], TQB[11]);
  buf B552(TQB_[12], TQB[12]);
  buf B553(TQB_[13], TQB[13]);
  buf B554(TQB_[14], TQB[14]);
  buf B555(TQB_[15], TQB[15]);
  buf B556(TQB_[16], TQB[16]);
  buf B557(TQB_[17], TQB[17]);
  buf B558(TQB_[18], TQB[18]);
  buf B559(TQB_[19], TQB[19]);
  buf B560(TQB_[20], TQB[20]);
  buf B561(TQB_[21], TQB[21]);
  buf B562(TQB_[22], TQB[22]);
  buf B563(TQB_[23], TQB[23]);
  buf B564(TQB_[24], TQB[24]);
  buf B565(TQB_[25], TQB[25]);
  buf B566(TQB_[26], TQB[26]);
  buf B567(TQB_[27], TQB[27]);
  buf B568(TQB_[28], TQB[28]);
  buf B569(TQB_[29], TQB[29]);
  buf B570(TQB_[30], TQB[30]);
  buf B571(TQB_[31], TQB[31]);
  buf B572(TQB_[32], TQB[32]);
  buf B573(TQB_[33], TQB[33]);
  buf B574(TQB_[34], TQB[34]);
  buf B575(TQB_[35], TQB[35]);
  buf B576(TQB_[36], TQB[36]);
  buf B577(TQB_[37], TQB[37]);
  buf B578(TQB_[38], TQB[38]);
  buf B579(TQB_[39], TQB[39]);
  buf B580(TQB_[40], TQB[40]);
  buf B581(TQB_[41], TQB[41]);
  buf B582(TQB_[42], TQB[42]);
  buf B583(TQB_[43], TQB[43]);
  buf B584(TQB_[44], TQB[44]);
  buf B585(TQB_[45], TQB[45]);
  buf B586(TQB_[46], TQB[46]);
  buf B587(TQB_[47], TQB[47]);
  buf B588(TQB_[48], TQB[48]);
  buf B589(TQB_[49], TQB[49]);
  buf B590(TQB_[50], TQB[50]);
  buf B591(TQB_[51], TQB[51]);
  buf B592(RET1N_, RET1N);
  buf B593(STOVA_, STOVA);
  buf B594(STOVB_, STOVB);
  buf B595(COLLDISN_, COLLDISN);
  ...
  assign DYA_ = RET1N_ ? (TENA_ ? DA_ : TDA_) : {52{1'bx}};
  ...
  assign DYB_ = RET1N_ ? (TENB_ ? DB_ : TDB_) : {52{1'bx}};
   `ifdef ARM_FAULT_MODELING
     sram_keybuf1_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .TQ(TQA_), .BEN(BENA_), .WEN(WENA_int), .Q_in(QA_int));
  `else
  assign QA_ = RET1N_ ? (BENA_ ? ((STOVA_ ? (QA_int_delayed) : (QA_int))) : TQA_) : {52{1'bx}};
  `endif
  assign QB_ = RET1N_ ? (BENB_ ? ((STOVB_ ? (QB_int_delayed) : (QB_int))) : TQB_) : {52{1'bx}};
...
`endif
`timescale 1ns/1ps
module sram_keybuf1_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, BEN, TQ);
   output [51:0] Q_out;
   input [51:0] Q_in;
   input CLK;
   input [6:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(7'd2,6'd22,2'd1,2'd0);
   `endif
end
   task add_fault;
   //This task injects fault in memory
   //In order to inject fault in redundant column for Bit 0 to 25, column address
   //should have value in range of 4 to 7
   //In order to inject fault in redundant column for Bit 26 to 51, column address
