...
//       
//       Creation Date:  Mon Jan  9 16:03:35 2023
//       Version: 	r5p0
//
...
`ifdef POWER_PINS
module sram_FAST_lb (VDDCE, VDDPE, VSSE, CENYA, WENYA, AYA, DYA, CENYB, WENYB, AYB,
...
`else
module sram_FAST_lb (CENYA, WENYA, AYA, DYA, CENYB, WENYB, AYB, DYB, QA, QB, CLKA,
...
`endif
  ...
  reg LAST_CLKB;
  reg [17:0] QA_int;
  reg [17:0] QA_int_delayed;
  reg [17:0] QB_int;
  reg [17:0] QB_int_delayed;
  ...
  reg NOT_AA2, NOT_AA1, NOT_AA0, NOT_DA17, NOT_DA16, NOT_DA15, NOT_DA14, NOT_DA13;
  ...
  reg NOT_TAA1, NOT_TAA0, NOT_TDA17, NOT_TDA16, NOT_TDA15, NOT_TDA14, NOT_TDA13, NOT_TDA12;
  ...
  reg NOT_TAB0, NOT_TDB17, NOT_TDB16, NOT_TDB15, NOT_TDB14, NOT_TDB13, NOT_TDB12, NOT_TDB11;
 ...
 wire  CLKB_;
  ...
  assign `ARM_UD_DP DYA_ = RET1N_ ? (TENA_ ? DA_ : TDA_) : {18{1'bx}};
  ...
  assign `ARM_UD_DP DYB_ = RET1N_ ? (TENB_ ? DB_ : TDB_) : {18{1'bx}};
  assign `ARM_UD_SEQ QA_ = RET1N_ ? (BENA_ ? ((STOVA_ ? (QA_int_delayed) : (QA_int))) : TQA_) : {18{1'bx}};
  assign `ARM_UD_SEQ QB_ = RET1N_ ? (BENB_ ? ((STOVB_ ? (QB_int_delayed) : (QB_int))) : TQB_) : {18{1'bx}};
  ...
  begin
	$readmemb(filename, memld);
     if (CENA_ === 1'b1 && CENB_ === 1'b1) begin
      ...
      row = mem[row_address];
        writeEnable = {18{1'b1}};
        row_mask =  ( {15'b000000000000000, writeEnable[17], 15'b000000000000000, writeEnable[16],
        ...
        new_data =  ( {15'b000000000000000, wordtemp[17], 15'b000000000000000, wordtemp[16],
  ...
  begin
	dump_file_desc = $fopen(filename_dump);
     if (CENA_ === 1'b1 && CENB_ === 1'b1) begin
      ...
      QA_int = {shifted_readLatch0[68], shifted_readLatch0[64], shifted_readLatch0[60],
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
     ...
     && !CENA_int)} === 1'bx) begin
      ...
      QA_int = WENA_int !== 1'b1 ? QA_int : {18{1'bx}};
      QA_int_delayed = WENA_int !== 1'b1 ? QA_int_delayed : {18{1'bx}};
      ...
      if (WENA_int !== 1'b1) begin
        row_mask =  ( {15'b000000000000000, writeEnable[17], 15'b000000000000000, writeEnable[16],
          ...
          15'b000000000000000, writeEnable[3], 15'b000000000000000, writeEnable[2],
          15'b000000000000000, writeEnable[1], 15'b000000000000000, writeEnable[0]} << mux_address);
        new_data =  ( {15'b000000000000000, DA_int[17], 15'b000000000000000, DA_int[16],
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
     ...
     && !CENB_int)} === 1'bx) begin
      ...
      QB_int = WENB_int !== 1'b1 ? QB_int : {18{1'bx}};
      QB_int_delayed = WENB_int !== 1'b1 ? QB_int_delayed : {18{1'bx}};
      ...
      if (WENB_int !== 1'b1) begin
        row_mask =  ( {15'b000000000000000, writeEnable[17], 15'b000000000000000, writeEnable[16],
          ...
          15'b000000000000000, writeEnable[3], 15'b000000000000000, writeEnable[2],
          15'b000000000000000, writeEnable[1], 15'b000000000000000, writeEnable[0]} << mux_address);
        new_data =  ( {15'b000000000000000, DB_int[17], 15'b000000000000000, DB_int[16],
      ...
      QB_int = {shifted_readLatch1[68], shifted_readLatch1[64], shifted_readLatch1[60],
...
`endif
 ...
 wire  CLKB_;
  ...
  buf B30(CENYB, CENYB_);
  buf B31(WENYB, WENYB_);
  buf B32(AYB[0], AYB_[0]);
  buf B33(AYB[1], AYB_[1]);
  buf B34(AYB[2], AYB_[2]);
  buf B35(AYB[3], AYB_[3]);
  buf B36(AYB[4], AYB_[4]);
  buf B37(AYB[5], AYB_[5]);
  buf B38(AYB[6], AYB_[6]);
  buf B39(AYB[7], AYB_[7]);
  buf B40(AYB[8], AYB_[8]);
  buf B41(AYB[9], AYB_[9]);
  ...
  buf B68(QA[8], QA_[8]);
  buf B69(QA[9], QA_[9]);
  buf B70(QA[10], QA_[10]);
  buf B71(QA[11], QA_[11]);
  buf B72(QA[12], QA_[12]);
  buf B73(QA[13], QA_[13]);
  buf B74(QA[14], QA_[14]);
  buf B75(QA[15], QA_[15]);
  buf B76(QA[16], QA_[16]);
  buf B77(QA[17], QA_[17]);
  buf B78(QB[0], QB_[0]);
  buf B79(QB[1], QB_[1]);
  buf B80(QB[2], QB_[2]);
  buf B81(QB[3], QB_[3]);
  ...
  buf B86(QB[8], QB_[8]);
  buf B87(QB[9], QB_[9]);
  buf B88(QB[10], QB_[10]);
  buf B89(QB[11], QB_[11]);
  buf B90(QB[12], QB_[12]);
  buf B91(QB[13], QB_[13]);
  buf B92(QB[14], QB_[14]);
  buf B93(QB[15], QB_[15]);
  buf B94(QB[16], QB_[16]);
  buf B95(QB[17], QB_[17]);
  buf B96(CLKA_, CLKA);
  buf B97(CENA_, CENA);
  buf B98(WENA_, WENA);
  buf B99(AA_[0], AA[0]);
  ...
  buf B117(DA_[8], DA[8]);
  buf B118(DA_[9], DA[9]);
  buf B119(DA_[10], DA[10]);
  buf B120(DA_[11], DA[11]);
  buf B121(DA_[12], DA[12]);
  buf B122(DA_[13], DA[13]);
  buf B123(DA_[14], DA[14]);
  buf B124(DA_[15], DA[15]);
  buf B125(DA_[16], DA[16]);
  buf B126(DA_[17], DA[17]);
  buf B127(CLKB_, CLKB);
  buf B128(CENB_, CENB);
  buf B129(WENB_, WENB);
  buf B130(AB_[0], AB[0]);
  ...
  buf B148(DB_[8], DB[8]);
  buf B149(DB_[9], DB[9]);
  buf B150(DB_[10], DB[10]);
  buf B151(DB_[11], DB[11]);
  buf B152(DB_[12], DB[12]);
  buf B153(DB_[13], DB[13]);
  buf B154(DB_[14], DB[14]);
  buf B155(DB_[15], DB[15]);
  buf B156(DB_[16], DB[16]);
  buf B157(DB_[17], DB[17]);
  buf B158(EMAA_[0], EMAA[0]);
  buf B159(EMAA_[1], EMAA[1]);
  buf B160(EMAA_[2], EMAA[2]);
  buf B161(EMAWA_[0], EMAWA[0]);
  buf B162(EMAWA_[1], EMAWA[1]);
  buf B163(EMASA_, EMASA);
  buf B164(EMAB_[0], EMAB[0]);
  buf B165(EMAB_[1], EMAB[1]);
  buf B166(EMAB_[2], EMAB[2]);
  buf B167(EMAWB_[0], EMAWB[0]);
  buf B168(EMAWB_[1], EMAWB[1]);
  buf B169(EMASB_, EMASB);
  buf B170(TENA_, TENA);
  buf B171(BENA_, BENA);
  buf B172(TCENA_, TCENA);
  buf B173(TWENA_, TWENA);
  buf B174(TAA_[0], TAA[0]);
  buf B175(TAA_[1], TAA[1]);
  buf B176(TAA_[2], TAA[2]);
  buf B177(TAA_[3], TAA[3]);
  buf B178(TAA_[4], TAA[4]);
  buf B179(TAA_[5], TAA[5]);
  buf B180(TAA_[6], TAA[6]);
  buf B181(TAA_[7], TAA[7]);
  buf B182(TAA_[8], TAA[8]);
  buf B183(TAA_[9], TAA[9]);
  buf B184(TDA_[0], TDA[0]);
  buf B185(TDA_[1], TDA[1]);
  buf B186(TDA_[2], TDA[2]);
  buf B187(TDA_[3], TDA[3]);
  buf B188(TDA_[4], TDA[4]);
  buf B189(TDA_[5], TDA[5]);
  buf B190(TDA_[6], TDA[6]);
  buf B191(TDA_[7], TDA[7]);
  buf B192(TDA_[8], TDA[8]);
  buf B193(TDA_[9], TDA[9]);
  buf B194(TDA_[10], TDA[10]);
  buf B195(TDA_[11], TDA[11]);
  buf B196(TDA_[12], TDA[12]);
  buf B197(TDA_[13], TDA[13]);
  buf B198(TDA_[14], TDA[14]);
  buf B199(TDA_[15], TDA[15]);
  buf B200(TDA_[16], TDA[16]);
  buf B201(TDA_[17], TDA[17]);
  buf B202(TQA_[0], TQA[0]);
  buf B203(TQA_[1], TQA[1]);
  buf B204(TQA_[2], TQA[2]);
  buf B205(TQA_[3], TQA[3]);
  buf B206(TQA_[4], TQA[4]);
  buf B207(TQA_[5], TQA[5]);
  buf B208(TQA_[6], TQA[6]);
  buf B209(TQA_[7], TQA[7]);
  buf B210(TQA_[8], TQA[8]);
  buf B211(TQA_[9], TQA[9]);
  buf B212(TQA_[10], TQA[10]);
  buf B213(TQA_[11], TQA[11]);
  buf B214(TQA_[12], TQA[12]);
  buf B215(TQA_[13], TQA[13]);
  buf B216(TQA_[14], TQA[14]);
  buf B217(TQA_[15], TQA[15]);
  buf B218(TQA_[16], TQA[16]);
  buf B219(TQA_[17], TQA[17]);
  buf B220(TENB_, TENB);
  buf B221(BENB_, BENB);
  buf B222(TCENB_, TCENB);
  buf B223(TWENB_, TWENB);
  buf B224(TAB_[0], TAB[0]);
  buf B225(TAB_[1], TAB[1]);
  buf B226(TAB_[2], TAB[2]);
  buf B227(TAB_[3], TAB[3]);
  buf B228(TAB_[4], TAB[4]);
  buf B229(TAB_[5], TAB[5]);
  buf B230(TAB_[6], TAB[6]);
  buf B231(TAB_[7], TAB[7]);
  buf B232(TAB_[8], TAB[8]);
  buf B233(TAB_[9], TAB[9]);
  buf B234(TDB_[0], TDB[0]);
  buf B235(TDB_[1], TDB[1]);
  buf B236(TDB_[2], TDB[2]);
  buf B237(TDB_[3], TDB[3]);
  buf B238(TDB_[4], TDB[4]);
  buf B239(TDB_[5], TDB[5]);
  buf B240(TDB_[6], TDB[6]);
  buf B241(TDB_[7], TDB[7]);
  buf B242(TDB_[8], TDB[8]);
  buf B243(TDB_[9], TDB[9]);
  buf B244(TDB_[10], TDB[10]);
  buf B245(TDB_[11], TDB[11]);
  buf B246(TDB_[12], TDB[12]);
  buf B247(TDB_[13], TDB[13]);
  buf B248(TDB_[14], TDB[14]);
  buf B249(TDB_[15], TDB[15]);
  buf B250(TDB_[16], TDB[16]);
  buf B251(TDB_[17], TDB[17]);
  buf B252(TQB_[0], TQB[0]);
  buf B253(TQB_[1], TQB[1]);
  buf B254(TQB_[2], TQB[2]);
  buf B255(TQB_[3], TQB[3]);
  buf B256(TQB_[4], TQB[4]);
  buf B257(TQB_[5], TQB[5]);
  buf B258(TQB_[6], TQB[6]);
  buf B259(TQB_[7], TQB[7]);
  buf B260(TQB_[8], TQB[8]);
  buf B261(TQB_[9], TQB[9]);
  buf B262(TQB_[10], TQB[10]);
  buf B263(TQB_[11], TQB[11]);
  buf B264(TQB_[12], TQB[12]);
  buf B265(TQB_[13], TQB[13]);
  buf B266(TQB_[14], TQB[14]);
  buf B267(TQB_[15], TQB[15]);
  buf B268(TQB_[16], TQB[16]);
  buf B269(TQB_[17], TQB[17]);
  buf B270(RET1N_, RET1N);
  buf B271(STOVA_, STOVA);
  buf B272(STOVB_, STOVB);
  buf B273(COLLDISN_, COLLDISN);
  ...
  assign DYA_ = RET1N_ ? (TENA_ ? DA_ : TDA_) : {18{1'bx}};
  ...
  assign DYB_ = RET1N_ ? (TENB_ ? DB_ : TDB_) : {18{1'bx}};
   `ifdef ARM_FAULT_MODELING
     sram_FAST_lb_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .TQ(TQA_), .BEN(BENA_), .WEN(WENA_int), .Q_in(QA_int));
  `else
  assign QA_ = RET1N_ ? (BENA_ ? ((STOVA_ ? (QA_int_delayed) : (QA_int))) : TQA_) : {18{1'bx}};
  `endif
  assign QB_ = RET1N_ ? (BENB_ ? ((STOVB_ ? (QB_int_delayed) : (QB_int))) : TQB_) : {18{1'bx}};
...
`endif
`timescale 1ns/1ps
module sram_FAST_lb_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, BEN, TQ);
   output [17:0] Q_out;
   input [17:0] Q_in;
   input CLK;
   input [9:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(10'd37,5'd16,2'd1,2'd0);
   `endif
end
   task add_fault;
   //This task injects fault in memory
   //In order to inject fault in redundant column for Bit 0 to 8, column address
   //should have value in range of 12 to 15
   //In order to inject fault in redundant column for Bit 9 to 17, column address
