...
//       
//       Creation Date:  Fri Nov  4 16:36:27 2022
//       Version: 	r5p0
//
...
`ifdef POWER_PINS
module sram_dp_sincos (VDDCE, VDDPE, VSSE, CENYA, WENYA, AYA, DYA, CENYB, WENYB, AYB,
...
`else
module sram_dp_sincos (CENYA, WENYA, AYA, DYA, CENYB, WENYB, AYB, DYB, QA, QB, CLKA, CENA,
...
`endif
  ...
  parameter WORDS = 640;
  parameter MUX = 16;
  parameter MEM_WIDTH = 192; // redun block size 4, 96 on left, 96 on right
...
`endif
  ...
  reg [47:0] readLatch1;
  reg [47:0] shifted_readLatch1;
  reg [1:0] read_mux_sel1;
  reg LAST_CLKB;
  reg [11:0] QA_int;
  reg [11:0] QA_int_delayed;
  reg [11:0] QB_int;
  reg [11:0] QB_int_delayed;
  ...
  reg NOT_AA2, NOT_AA1, NOT_AA0, NOT_DA11, NOT_DA10, NOT_DA9, NOT_DA8, NOT_DA7, NOT_DA6;
  reg NOT_DA5, NOT_DA4, NOT_DA3, NOT_DA2, NOT_DA1, NOT_DA0, NOT_CENB, NOT_WENB, NOT_AB9;
  reg NOT_AB8, NOT_AB7, NOT_AB6, NOT_AB5, NOT_AB4, NOT_AB3, NOT_AB2, NOT_AB1, NOT_AB0;
  ...
  reg NOT_TAA4, NOT_TAA3, NOT_TAA2, NOT_TAA1, NOT_TAA0, NOT_TDA11, NOT_TDA10, NOT_TDA9;
  reg NOT_TDA8, NOT_TDA7, NOT_TDA6, NOT_TDA5, NOT_TDA4, NOT_TDA3, NOT_TDA2, NOT_TDA1;
  reg NOT_TDA0, NOT_TENB, NOT_TCENB, NOT_TWENB, NOT_TAB9, NOT_TAB8, NOT_TAB7, NOT_TAB6;
  reg NOT_TAB5, NOT_TAB4, NOT_TAB3, NOT_TAB2, NOT_TAB1, NOT_TAB0, NOT_TDB11, NOT_TDB10;
 ...
 wire  CLKB_;
  ...
  assign `ARM_UD_DP DYA_ = RET1N_ ? (TENA_ ? DA_ : TDA_) : {12{1'bx}};
  ...
  assign `ARM_UD_DP DYB_ = RET1N_ ? (TENB_ ? DB_ : TDB_) : {12{1'bx}};
  assign `ARM_UD_SEQ QA_ = RET1N_ ? (BENA_ ? ((STOVA_ ? (QA_int_delayed) : (QA_int))) : TQA_) : {12{1'bx}};
  assign `ARM_UD_SEQ QB_ = RET1N_ ? (BENB_ ? ((STOVB_ ? (QB_int_delayed) : (QB_int))) : TQB_) : {12{1'bx}};
  ...
  begin
	$readmemb(filename, memld);
     if (CENA_ === 1'b1 && CENB_ === 1'b1) begin
      ...
      row = mem[row_address];
        writeEnable = {12{1'b1}};
        row_mask =  ( {15'b000000000000000, writeEnable[11], 15'b000000000000000, writeEnable[10],
        ...
        new_data =  ( {15'b000000000000000, wordtemp[11], 15'b000000000000000, wordtemp[10],
	...
	dump_file_desc = $fopen(filename_dump);
     if (CENA_ === 1'b1 && CENB_ === 1'b1) begin
      ...
      row = mem[row_address];
        ...
        readLatch0 = {data_out[188], data_out[184], data_out[180], data_out[176], data_out[172],
      ...
      QA_int = {shifted_readLatch0[44], shifted_readLatch0[40], shifted_readLatch0[36],
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
     ...
     && !CENA_int)} === 1'bx) begin
      ...
      QA_int = WENA_int !== 1'b1 ? QA_int : {12{1'bx}};
      QA_int_delayed = WENA_int !== 1'b1 ? QA_int_delayed : {12{1'bx}};
      ...
      if (WENA_int !== 1'b1) begin
        row_mask =  ( {15'b000000000000000, writeEnable[11], 15'b000000000000000, writeEnable[10],
          ...
          15'b000000000000000, writeEnable[3], 15'b000000000000000, writeEnable[2],
          15'b000000000000000, writeEnable[1], 15'b000000000000000, writeEnable[0]} << mux_address);
        new_data =  ( {15'b000000000000000, DA_int[11], 15'b000000000000000, DA_int[10],
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
     ...
     && !CENB_int)} === 1'bx) begin
      ...
      QB_int = WENB_int !== 1'b1 ? QB_int : {12{1'bx}};
      QB_int_delayed = WENB_int !== 1'b1 ? QB_int_delayed : {12{1'bx}};
      ...
      if (WENB_int !== 1'b1) begin
        row_mask =  ( {15'b000000000000000, writeEnable[11], 15'b000000000000000, writeEnable[10],
          ...
          15'b000000000000000, writeEnable[3], 15'b000000000000000, writeEnable[2],
          15'b000000000000000, writeEnable[1], 15'b000000000000000, writeEnable[0]} << mux_address);
        new_data =  ( {15'b000000000000000, DB_int[11], 15'b000000000000000, DB_int[10],
        ...
        data_out = (row >> (mux_address%4));
        readLatch1 = {data_out[188], data_out[184], data_out[180], data_out[176], data_out[172],
          data_out[168], data_out[164], data_out[160], data_out[156], data_out[152],
          data_out[148], data_out[144], data_out[140], data_out[136], data_out[132],
          data_out[128], data_out[124], data_out[120], data_out[116], data_out[112],
          data_out[108], data_out[104], data_out[100], data_out[96], data_out[92],
      ...
      QB_int = {shifted_readLatch1[44], shifted_readLatch1[40], shifted_readLatch1[36],
...
`endif
 ...
 wire  CLKB_;
  ...
  buf B24(CENYB, CENYB_);
  buf B25(WENYB, WENYB_);
  buf B26(AYB[0], AYB_[0]);
  buf B27(AYB[1], AYB_[1]);
  buf B28(AYB[2], AYB_[2]);
  buf B29(AYB[3], AYB_[3]);
  buf B30(AYB[4], AYB_[4]);
  buf B31(AYB[5], AYB_[5]);
  buf B32(AYB[6], AYB_[6]);
  buf B33(AYB[7], AYB_[7]);
  buf B34(AYB[8], AYB_[8]);
  buf B35(AYB[9], AYB_[9]);
  buf B36(DYB[0], DYB_[0]);
  buf B37(DYB[1], DYB_[1]);
  buf B38(DYB[2], DYB_[2]);
  buf B39(DYB[3], DYB_[3]);
  buf B40(DYB[4], DYB_[4]);
  buf B41(DYB[5], DYB_[5]);
  buf B42(DYB[6], DYB_[6]);
  buf B43(DYB[7], DYB_[7]);
  buf B44(DYB[8], DYB_[8]);
  buf B45(DYB[9], DYB_[9]);
  buf B46(DYB[10], DYB_[10]);
  buf B47(DYB[11], DYB_[11]);
  buf B48(QA[0], QA_[0]);
  buf B49(QA[1], QA_[1]);
  buf B50(QA[2], QA_[2]);
  buf B51(QA[3], QA_[3]);
  ...
  buf B56(QA[8], QA_[8]);
  buf B57(QA[9], QA_[9]);
  buf B58(QA[10], QA_[10]);
  buf B59(QA[11], QA_[11]);
  buf B60(QB[0], QB_[0]);
  buf B61(QB[1], QB_[1]);
  buf B62(QB[2], QB_[2]);
  buf B63(QB[3], QB_[3]);
  ...
  buf B93(DA_[8], DA[8]);
  buf B94(DA_[9], DA[9]);
  buf B95(DA_[10], DA[10]);
  buf B96(DA_[11], DA[11]);
  buf B97(CLKB_, CLKB);
  buf B98(CENB_, CENB);
  buf B99(WENB_, WENB);
  buf B100(AB_[0], AB[0]);
  ...
  buf B122(EMAA_[0], EMAA[0]);
  buf B123(EMAA_[1], EMAA[1]);
  buf B124(EMAA_[2], EMAA[2]);
  buf B125(EMAWA_[0], EMAWA[0]);
  buf B126(EMAWA_[1], EMAWA[1]);
  buf B127(EMASA_, EMASA);
  buf B128(EMAB_[0], EMAB[0]);
  buf B129(EMAB_[1], EMAB[1]);
  buf B130(EMAB_[2], EMAB[2]);
  buf B131(EMAWB_[0], EMAWB[0]);
  buf B132(EMAWB_[1], EMAWB[1]);
  buf B133(EMASB_, EMASB);
  buf B134(TENA_, TENA);
  buf B135(BENA_, BENA);
  buf B136(TCENA_, TCENA);
  buf B137(TWENA_, TWENA);
  ...
  buf B156(TDA_[8], TDA[8]);
  buf B157(TDA_[9], TDA[9]);
  buf B158(TDA_[10], TDA[10]);
  buf B159(TDA_[11], TDA[11]);
  buf B160(TQA_[0], TQA[0]);
  buf B161(TQA_[1], TQA[1]);
  buf B162(TQA_[2], TQA[2]);
  buf B163(TQA_[3], TQA[3]);
  buf B164(TQA_[4], TQA[4]);
  buf B165(TQA_[5], TQA[5]);
  buf B166(TQA_[6], TQA[6]);
  buf B167(TQA_[7], TQA[7]);
  buf B168(TQA_[8], TQA[8]);
  buf B169(TQA_[9], TQA[9]);
  buf B170(TQA_[10], TQA[10]);
  buf B171(TQA_[11], TQA[11]);
  buf B172(TENB_, TENB);
  buf B173(BENB_, BENB);
  buf B174(TCENB_, TCENB);
  buf B175(TWENB_, TWENB);
  buf B176(TAB_[0], TAB[0]);
  buf B177(TAB_[1], TAB[1]);
  buf B178(TAB_[2], TAB[2]);
  buf B179(TAB_[3], TAB[3]);
  buf B180(TAB_[4], TAB[4]);
  buf B181(TAB_[5], TAB[5]);
  buf B182(TAB_[6], TAB[6]);
  buf B183(TAB_[7], TAB[7]);
  buf B184(TAB_[8], TAB[8]);
  buf B185(TAB_[9], TAB[9]);
  buf B186(TDB_[0], TDB[0]);
  buf B187(TDB_[1], TDB[1]);
  buf B188(TDB_[2], TDB[2]);
  buf B189(TDB_[3], TDB[3]);
  buf B190(TDB_[4], TDB[4]);
  buf B191(TDB_[5], TDB[5]);
  buf B192(TDB_[6], TDB[6]);
  buf B193(TDB_[7], TDB[7]);
  buf B194(TDB_[8], TDB[8]);
  buf B195(TDB_[9], TDB[9]);
  buf B196(TDB_[10], TDB[10]);
  buf B197(TDB_[11], TDB[11]);
  buf B198(TQB_[0], TQB[0]);
  buf B199(TQB_[1], TQB[1]);
  buf B200(TQB_[2], TQB[2]);
  buf B201(TQB_[3], TQB[3]);
  buf B202(TQB_[4], TQB[4]);
  buf B203(TQB_[5], TQB[5]);
  buf B204(TQB_[6], TQB[6]);
  buf B205(TQB_[7], TQB[7]);
  buf B206(TQB_[8], TQB[8]);
  buf B207(TQB_[9], TQB[9]);
  buf B208(TQB_[10], TQB[10]);
  buf B209(TQB_[11], TQB[11]);
  buf B210(RET1N_, RET1N);
  buf B211(STOVA_, STOVA);
  buf B212(STOVB_, STOVB);
  buf B213(COLLDISN_, COLLDISN);
  ...
  assign DYA_ = RET1N_ ? (TENA_ ? DA_ : TDA_) : {12{1'bx}};
  ...
  assign DYB_ = RET1N_ ? (TENB_ ? DB_ : TDB_) : {12{1'bx}};
   `ifdef ARM_FAULT_MODELING
     sram_dp_sincos_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .TQ(TQA_), .BEN(BENA_), .WEN(WENA_int), .Q_in(QA_int));
  `else
  assign QA_ = RET1N_ ? (BENA_ ? ((STOVA_ ? (QA_int_delayed) : (QA_int))) : TQA_) : {12{1'bx}};
  `endif
  assign QB_ = RET1N_ ? (BENB_ ? ((STOVB_ ? (QB_int_delayed) : (QB_int))) : TQB_) : {12{1'bx}};
...
`endif
`timescale 1ns/1ps
module sram_dp_sincos_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, BEN, TQ);
   output [11:0] Q_out;
   input [11:0] Q_in;
   input CLK;
   input [9:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(10'd37,4'd10,2'd1,2'd0);
   `endif
end
   task add_fault;
